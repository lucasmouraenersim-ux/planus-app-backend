...
import { doc, getDoc, setDoc, collection, getDocs, Timestamp, updateDoc, query, orderBy, onSnapshot as onFirestoreSnapshot } from 'firebase/firestore';
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const userStatusDocRef = doc(db, 'status', firebaseUser.uid);
    ...
    updateStatus(true); // User is online
  ...
  useEffect(() => {
    // Listen to the 'status' collection for real-time updates
    const statusCollectionRef = collection(db, 'status');
    const unsubscribe = onFirestoreSnapshot(statusCollectionRef, (snapshot) => {
        const statuses: Record<string, 'online' | 'offline'> = {};
        snapshot.forEach((doc) => {
            statuses[doc.id] = doc.data().state;
        });
