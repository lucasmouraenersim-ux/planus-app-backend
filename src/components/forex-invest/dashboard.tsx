...
import { CalendarIcon, DollarSign, TrendingDown, TrendingUp, LayoutGrid, List } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
        <div className="flex justify-between items-center">
            <h1 className="text-3xl font-bold font-serif text-primary">Dashboard de Projeção</h1>
            <div className="flex gap-2">
                ...
                <Button variant="outline" onClick={() => setDateRange('last7')}>7 Dias</Button>
                <Button variant="outline" onClick={() => setDateRange('last30')}>30 Dias</Button>
                <Button variant="outline" onClick={() => setDateRange('all')}>Todo Período</Button>
            </div>
        </div>
        ...
        <Tabs defaultValue="dashboard">
            <TabsList className="mb-4">
                ...
                <TabsTrigger value="projection"><List className="mr-2 h-4 w-4"/>Tabela de Projeção</TabsTrigger>
            </TabsList>
            <TabsContent value="dashboard" className="space-y-6">
                 <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    <Card className="bg-card/70 backdrop-blur-lg border shadow-lg"><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Lucro/Prejuízo Total</CardTitle><DollarSign className="h-4 w-4 text-muted-foreground" /></CardHeader>
                    ...
                    <Card className="bg-card/70 backdrop-blur-lg border shadow-lg"><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Evolução Diária Média</CardTitle><TrendingUp className="h-4 w-4 text-muted-foreground" /></CardHeader>
                      <CardContent><div className="text-2xl font-bold">{dailyPerformance.avgEvolution.toFixed(2)}%</div></CardContent></Card>
                    <Card className="bg-card/70 backdrop-blur-lg border shadow-lg"><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Maior Ganho Diário</CardTitle><TrendingUp className="h-4 w-4 text-muted-foreground" /></CardHeader>
                      <CardContent><div className="text-2xl font-bold text-green-500">${dailyPerformance.bestDay.toFixed(2)}</div></CardContent></Card>
                    <Card className="bg-card/70 backdrop-blur-lg border shadow-lg"><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Pior Perda Diária</CardTitle><TrendingDown className="h-4 w-4 text-muted-foreground" /></CardHeader>
