  ...
  useEffect(() => {
    ...
    const calculatedReceivables: Receivable[] = receivableLeads.map(lead => {
      ...
      if (empresa === 'Fit Energia') {
        ...
        secondCommission = proposta * 0.35;
                          ...
                          {filteredUsers.map(user => {
                            ...
                            return (
                              <TableRow key={user.uid}>
                                  ...
                                  <TableCell>
                                      <Switch checked={!user.disabled} onCheckedChange={async (checked) => { if(!canEdit) return; setIsSubmittingAction(true); try { const adminAction = (await import('@/actions/admin/userStatus')).updateUserStatus; await adminAction(user.uid, !checked); await onUsersChange(); toast({ title: "Status Alterado", description: `O usuário ${user.displayName} foi ${!checked ? 'desativado' : 'ativado'}.` }); } catch (e) { toast({ title: "Erro", description: "Falha ao alterar status.", variant: "destructive" }); } finally { setIsSubmittingAction(false); } }} disabled={!canEdit || isSubmittingAction} />
                                  </TableCell>
