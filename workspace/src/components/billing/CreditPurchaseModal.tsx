...
export function CreditPurchaseModal({ isOpen, onClose }: CreditPurchaseModalProps) {
  ...
  // Verifica a data no front também para mostrar o preço certo
  const hoje = new Date();
  const dataVirada = new Date('2025-12-26T00:00:00');
  ...
  const PlanCard = ({ id, credits, price, label, bestValue, isSub }: any) => (
    <div className={`relative border p-4 rounded-xl flex flex-col gap-3 transition-all cursor-pointer hover:border-cyan-500/50 hover:bg-slate-800/50 ${bestValue ? 'border-yellow-500/50 bg-yellow-900/10' : 'border-white/10 bg-slate-900'}`}>
        ...
        <div className="mt-auto">
            <div className="text-2xl font-bold text-white">R$ {price}<span className="text-sm font-normal text-slate-500"></span></div>
        ...
        <div className="mt-4">
            <Tabs defaultValue="avulso" className="w-full">
                ...
                <TabsContent value="avulso" className="grid grid-cols-1 md:grid-cols-3 gap-4 animate-in fade-in slide-in-from-bottom-2">
                    <PlanCard 
                        id="pack_mini" 
                        credits={isPrecoNovo ? 20 : 10} 
                        price={isPrecoNovo ? "99,90" : "30,00"} 
                        label={isPrecoNovo ? "Ideal para Testes (R$ 4,99/cr)" : "Promoção de Natal"} 
                    />
                    <PlanCard 
                        id="pack_medio" 
                        credits={50} 
                        price={isPrecoNovo ? "199,90" : "125,00"} 
                        label={isPrecoNovo ? "Padrão (R$ 3,99/cr)" : "Econômico Natal"} 
                        bestValue={!isPrecoNovo}
                    />
                    <PlanCard 
                        id="pack_pro" 
                        credits={100} 
                        price={isPrecoNovo ? "299,90" : "200,00"} 
                        label="Profissional (R$ 2,99/cr)" 
                        bestValue={isPrecoNovo}
                    />
                </TabsContent>
