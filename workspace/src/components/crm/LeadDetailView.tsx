...
import { 
    ...
    CheckCircle, XCircle, AlertTriangle, X, Loader2, MessagesSquare, FileText, Banknote, UserSquare, Landmark, Download, Lightbulb, TrendingUp, Sparkles, BrainCircuit, Trash2
} from 'lucide-react';
import {
  AlertDialog,
  AlertDialogAction,
...
export function LeadDetailView({ lead, onClose, onEdit, isAdmin, onApprove, onRequestCorrection }: LeadDetailViewProps) {
  ...
  const handleClearChat = async () => {
    ...
    try {
      ...
      toast({ title: "Chat Limpo", description: "O histórico de mensagens foi apagado com sucesso." });
    } catch (error) {
      console.error("Error clearing chat history:", error);
      toast({ title: "Erro", description: "Não foi possível limpar o histórico de mensagens.", variant: "destructive" });
    }
  };
      ...
      <Card className="bg-card/80 backdrop-blur-xl border shadow-2xl flex flex-col h-[90vh]">
        ...
        <CardContent className="flex-1 flex flex-col min-h-0">
          ...
          <div className="flex-1 flex flex-col min-h-0 border-t pt-4">
            <h3 className="text-lg font-semibold text-foreground mb-2 flex items-center justify-between flex-shrink-0">
                <span className="flex items-center"><MessageSquare className="w-5 h-5 mr-2 text-primary"/>Chat com o Lead</span>
                {isAdmin && (
                    <AlertDialog>
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                    Esta ação é irreversível e irá apagar todas as mensagens desta conversa.
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
